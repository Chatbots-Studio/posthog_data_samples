...
Перед першою реальною відправкою скрипт **один раз** надсилає тестову подію `probe_connection` і перевіряє відповідь: має бути HTTP **200** і JSON **`{"status":"Ok"}`** (так PostHog підтверджує валідний [project API key](https://posthog.com/docs/api/overview#status-code-200)). Якщо прийшов HTML, порожня відповідь або інший JSON — з’єднання вважається невдалим (часто це невірний шлях або проксі не на той бекенд). Кожен **пакет** реальних подій перевіряється так само. Примусово без початкової перевірки: `node send-events.js --skip-verify` (пакети все одно валідуються по відповіді API).
